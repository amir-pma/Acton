...
actorDeclaration returns [ActorDeclaration actordec]
    :   a = ACTOR name = identifier {$actordec = new ActorDeclaration($name.id); $actordec.setLine($a.getLine());}
        ...
        LBRACE
            ...
            {
                VarDeclaration varDeclaration = new VarDeclaration($name.id, new ActorType($actor.id));
                varDeclaration.setLine($actor.id.getLine());
                $actordec.addKnownActor(varDeclaration);
            })*
        RBRACE)
